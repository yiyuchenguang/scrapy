...
class CpoemSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.browser.set_window_size(900, 600)  # 设置浏览器的窗体大小lf.browser = webdriver.Chrome(executable_path=browser_path,options=chrome_options)

    def parse(self, response):
        typecontent = response.xpath('.//div[@class ="sons"]//div[@class="typecont"]')
        for child in typecontent:
            bookMl = child.xpath('.//div[@class="bookMl"]/strong/text()').get()
            urls = child.xpath('.//span//a/@href').getall()
            for link in urls:
                next_url = response.urljoin(link)
                request = scrapy.Request(next_url, meta={'type':bookMl, 'gus':True}, callback=self.parse_item)
                yield request

    def parse_item(self, response):
        item = CPoemItem()
        item['poem_type'] = response.meta["type"]
        item['poem_url'] = response.url

        son = self.browser.find_element_by_xpath(".//div[@class='sons']")
        if son:
