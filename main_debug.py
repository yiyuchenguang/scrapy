...
class CompositePicture():
    ...
    def new_image(self,path,back_color):
        img = Image.open(path)
        # img.thumbnail((900, 500),Image.ANTIALIAS) # resize picture
        img = img.resize((900, 500), Image.ANTIALIAS)
        self.W = img.size[0]
        self.H = img.size[1]
        self.new_img = Image.new('RGB', (self.W, self.H * 2), back_color)  # 新建一张2倍高度的 白色底片的 图片
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for i in range(seg):
            ...
            pic_path="../image/3-1F930155254.jpg"
            ...
            #     C.draw.text((100, C.H * 2 - 70), random_value['poem_type'], font=setFont,fill="#239589")  # 利用ImageDraw的内置函数，在图片上写入文字
            #     poem_title = C.outPutStrLength(random_value['poem_title'], row_cut)  # 写入诗词标题
            #     C.draw_image(font_size= font_size + 8, fillColor="#D226DB", spaceSize=spaceSize, text=poem_title, offset= C.H + 20)
            #     poem_author = C.outPutStrLength(random_value['poem_author'], row_cut)  # 写入诗词作者
            #     C.draw_image(font_size= font_size - 10, fillColor="#3000f7", spaceSize= spaceSize, text=poem_author, offset=C.H +70)
            startpoint = s_num * i  # 每行的索引点
            #C.draw_image(font_size= font_size, fillColor="#239589", spaceSize=spaceSize, text=str(index),offset=C.H *2 - 30)  # 写入页脚
            C.draw_image(font_size=font_size, fillColor="#D226DB", spaceSize=spaceSize,text=retValue[startpoint:startpoint + s_num], offset=C.H + 100)  # 46:30
            C.new_img.show()
        ...
        #     startpoint = s_num * i  # 每行的索引点
        #     if i == 0:
        #         C.draw_image(font_size=font_size + 8, fillColor="#D226DB", spaceSize= spaceSize, text=["注释:"], offset=C.H + 20)
        #     C.draw_image(font_size= font_size, fillColor="#239589", spaceSize= spaceSize, text=str(index), offset=C.H *2 - 30)
        #     C.draw_image(font_size= font_size, fillColor="#D226DB", spaceSize= spaceSize, text= retValue[startpoint:startpoint+s_num],offset= C.H +80) # 46:30
        #     C.new_img.show()
        ...
        #     startpoint = s_num * i  # 每行的索引点
        #     if i == 0:
        #         C.draw_image(font_size=font_size + 8, fillColor="#3000f7", spaceSize= spaceSize, text=["赏析:"], offset=C.H + 20)# 标题
        #     C.draw_image(font_size= font_size, fillColor="#239589", spaceSize= spaceSize, text= str(index), offset=C.H *2 - 30)# 页脚
        #     C.draw_image(font_size= font_size, fillColor="#3000f7", spaceSize= spaceSize, text= retValue[startpoint:startpoint+s_num],offset= C.H +80) # 内容
        #     C.new_img.show()
