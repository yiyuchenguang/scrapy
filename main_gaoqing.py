...
class CompositePicture():
    def __init__(self, poem_json, fileDir,tarDir,poem_img_folder):
        self.font_type = "C:\\WINDOWS\\Fonts\\simfang.ttf" # 仿宋： simfang.TTF  ；黑体加粗：simhei.ttf
    ...
    def moveFile(self, sourceFile, targetFile):
        ...
        while (True):
            ...
            img = Image.open(targetFile + sample)
            if (1800<img.size[0] < 2000) and (1000<img.size[1] < 1201):
    ...
    def new_image(self,path,back_color):
        ...
        self.W = img.size[0]
        self.H = img.size[1]
        self.new_img = Image.new('RGB', (self.W, self.H + 1200), back_color)  # 新建一张2倍高度的 白色底片的 图片 ,默认1200pxi
...
if __name__ == '__main__':
    ...
    #C = CompositePicture(r"./c_poem/poem.json", r"./image/", r'./image_plant/', r"./out_tangshi/")  # 唐诗
    data = C.load()
    ...
    while True:
        ...
        # 诗词内容 ,
        retValue = C.outPutStrLength(random_value['poem_body'], 18)#宋词
        ...
        for i in range(seg):
            ...
            temp_img = pic_path.split('/')[-1].split('.')[0]  # eg: input"./keep_image/4-1Z916153311.jpg" , output 4-1Z916153311
            C.new_image(pic_path, '#EEE5E5')
            if (i == 0):
                setFont = ImageFont.truetype(C.font_type, 48)  # 写入诗词类型
                C.draw.text((100, C.H + 1200 - 70), random_value['poem_type'], font=setFont,fill="#239589")  # 利用ImageDraw的内置函数，在图片上写入文字
                poem_title = C.outPutStrLength(random_value['poem_title'], 36)  # 写入诗词标题
                C.draw_image(font_size=72, fillColor="#D226DB", spaceSize=36, text=poem_title, offset=C.H + 100)
                poem_author = C.outPutStrLength(random_value['poem_author'], 36)  # 写入诗词作者
                C.draw_image(font_size=48, fillColor="#3000f7", spaceSize=36, text=poem_author, offset=C.H + 200)
            startpoint = s_num * i  # 每行的索引点
            C.draw_image(font_size=72, fillColor="#239589", spaceSize=16, text=str(index),offset=C.H + 1200 - 70)  # 写入页脚
            C.draw_image(font_size=font_size, fillColor="#D226DB", spaceSize=spaceSize,text=retValue[startpoint:startpoint + s_num], offset=C.H + 300)  # 46:30
            #C.new_img.show()
            img_name = os.path.join(new_folder, "%s_%s_%d.gif" % (temp_img, new_title, i + 1))
            C.new_img.save(img_name, "gif")
        ...
        for i in range(seg):
            ...
            startpoint = s_num * i  # 每行的索引点
            if i == 0:
                C.draw_image(font_size=68, fillColor="#D226DB", spaceSize= spaceSize, text=["注释:"], offset=C.H + 30)
            C.draw_image(font_size= font_size, fillColor="#239589", spaceSize= spaceSize, text=str(index), offset=C.H +1200 - 70)
            C.draw_image(font_size= font_size, fillColor="#D226DB", spaceSize= spaceSize, text= retValue[startpoint:startpoint+s_num],offset= C.H +140) # 46:30
            #C.new_img.show()
            img_name = os.path.join(new_folder,"%s_%s_%s_%d.gif"%(temp_img,new_title,"注释",i+1))
            C.new_img.save(img_name, "gif")
            print("图片保存成功 ： %s" % img_name)
        ...
        for i in range(seg):
            ...
            startpoint = s_num * i  # 每行的索引点
            if i == 0:
                C.draw_image(font_size=68, fillColor="#3000f7", spaceSize= spaceSize, text=["赏析:"], offset=C.H + 30)# 标题
            C.draw_image(font_size= font_size, fillColor="#239589", spaceSize= spaceSize, text= str(index), offset=C.H +1200 - 70)# 页脚
            C.draw_image(font_size= font_size, fillColor="#3000f7", spaceSize= spaceSize, text= retValue[startpoint:startpoint+s_num],offset= C.H +140) # 内容
            #C.new_img.show()
            img_name = os.path.join(new_folder,"%s_%s_%s_%d.gif"%(temp_img,new_title,"赏析",i+1))
            C.new_img.save(img_name, "gif")
            print("图片保存成功 ： %s"%img_name)
        #break
